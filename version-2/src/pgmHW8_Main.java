...
public class pgmHW8_Main {
	...
	public static void main(String[] args) throws IOException {
		...
		File testFile = new File("/Users/briannahawkins/eclipse-workspace/pgmHW8/src/TestCases8.txt");
		// create Scanner object
		Scanner kybd = new Scanner(testFile);
		// open the output file
		PrintWriter outFile = new PrintWriter("/Users/briannahawkins/eclipse-workspace/pgmHW8/src/pgmOutput8.txt");
		...
		//System.out.println(String.valueOf(bank.getAccount(bank.acctNumberAt(0)).getNumTransactions()));
	}
	...
	public static void readAccts(Bank bank) throws IOException {
		File dbFile = new File("/Users/briannahawkins/eclipse-workspace/pgmHW8/src/initAccounts8.txt");
		...
		while (sc.hasNext()) {
			...
			Account account;
			String filename = "TransactionReceipts" + Integer.parseInt(tokens[4]) + ".dat";
	 ...
	 */
	public static void balance(Bank bank, Scanner kybd, PrintWriter outFile) throws IOException {
	 ...
	 */
	public static void deposit(Bank bank, Scanner kybd, PrintWriter outFile) throws IOException {
		...
		if (bank.findAcct(acctNum) != -1
				&& bank.getAccount(acctNum).getAcctType().equals("CD")) {
			...
			do {
				try {
					...
					case 24:
						...
						break;
					default:
						throw new CDMaturityDateException(MDate);
					}
				}
	 ...
	 * prints the transaction details
	 */
	public static void withdrawal(Bank bank, Scanner kybd, PrintWriter outFile) throws IOException {
		System.out.println("\nEnter account number: "); // prompt user for account number
		int acctNum = kybd.nextInt(); // read-in account number
		Calendar today = Calendar.getInstance(); // initialize today's date
		/*
	 ...
	 * TransactionReceipt receipt Output: prints the transaction details
	 */
	public static void clearCheck(Bank bank, Scanner kybd, PrintWriter outFile) throws IOException {
		System.out.println("\nEnter account number: "); // prompt user for account number
		int acctNum = kybd.nextInt(); // read-in account number
		Calendar today = Calendar.getInstance(); // initialize today's date
		/*
	 ...
	 * TransactionReceipt receipt Output: prints the transaction details
	 */
	public static void newAcct(Bank bank, Scanner kybd, PrintWriter outFile) throws IOException {
		System.out.println("\nEnter account number: "); // prompt user for account number
		int acctNum = kybd.nextInt(); // read-in account number
		Calendar today = Calendar.getInstance(); // initialize today's date
		/*
		...
		do {
			try {
				...
				case "Savings":
					...
					break;
				default:
					throw new InvalidAccountException(acctType);
				}
			}
	 ...
	 * prints the transaction details
	 */
	public static void deleteAcct(Bank bank, Scanner kybd, PrintWriter outFile) throws IOException {
		System.out.println("\nEnter account number: "); // prompt user for account number
		int acctNum = kybd.nextInt(); // read-in account number
		Calendar today = Calendar.getInstance(); // initialize today's date
		/*
	 ...
	 */
	public static void accountInfo(Bank bank, Scanner kybd, PrintWriter outFile) throws IOException {
	 ...
	 */
	public static void accountInfoHistory(Bank bank, Scanner kybd, PrintWriter outFile) throws IOException {
		...
		for (int i = 0; i < bank.getNumAccounts(); i++) { // traverse active database
			if (bank.getAccount(bank.acctNumberAt(i)).getDepositor().getSSN().equals(reqSSN)) { // account at the
				...
				for (int j = 0; j < bank.getAccount(bank.acctNumberAt(i)).getNumTransactions(); j++) { //traverse account transaction history
					// print complete transaction history for the transaction history at the current index
					outFile.println(bank.getAccount(bank.acctNumberAt(i)).TransactionReceiptAt(j).toString());
				}
				outFile.printf("\n%85s\n\n", "End of Transaction History for Account Number " + bank.acctNumberAt(i));
			}
			if (i == bank.getNumAccounts() - 1
					&& (!SSNFound)) { /*
										 * if the last index has been reached and the indicated SSN hasn't been found in
	 ...
	 * prints the transaction details
	 */
	public static void closeAcct(Bank bank, Scanner kybd, PrintWriter outFile) throws IOException {
	 ...
	 * prints the transaction details
	 */
	public static void reopenAcct(Bank bank, Scanner kybd, PrintWriter outFile) throws IOException {
